...
for url in htmls_names:# url = htmls_names[3421]
    ...
    if str(page)  != '<Response [404]>':
        ...
        if len(results_table) > 0:
            namefile = "../html/event_atp/{}_{}_{}_{}.html".format(url.split("/")[4], url.split("/")[5], url.split("/")[6], "singles")
    ...
    if str(page)  != '<Response [404]>':   
        ...
        results_table = tree.xpath("//div[@id='scoresResultsContent']")    
        if len(results_table) > 0:
            namefile = "../html/event_atp/{}_{}_{}_{}.html".format(url.split("/")[4], url.split("/")[5], url.split("/")[6], "doubles")
            html_file = open(namefile, 'w')
            html_file.write(page.text)
            html_file.close()
